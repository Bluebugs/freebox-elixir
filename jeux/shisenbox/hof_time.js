// helper for Hall Of Fame with time
...
function hof_to_hms(s)
{
    ...
    return hof_to0(h) + ':' + hof_to0(mn) + ':' + hof_to0(s);
}

// HOF setup
function hof_setup(game_name, backdrop, edje_o, evas, come_back_cb, scores_a, pos)
{
    hof_edje_o = edje_o;
    ...
    edje_object_file_set(hof_edje_o, "hof_time-" + game_name + ".edj", "hof");
...
function hof_setdown()
{
    ...
    edje_object_signal_emit(hof_edje_o,   "hof,time_col,disappear", "js");
...
function hof_set_score(score_o)
{
    ...
    edje_object_part_text_set(hof_edje_o, "time_" + rank, hof_to_hms(score_o.time));
    hof_emit_signal(rank, "appear")
}

function hof_emit_signal(rank, signal)
{
    edje_object_signal_emit(hof_edje_o,   "play,rank_" + rank + "," + signal, "js");
    edje_object_signal_emit(hof_edje_o,   "play,name_" + rank + "," + signal, "js");
    edje_object_signal_emit(hof_edje_o,   "play,time_" + rank + "," + signal, "js");
}
