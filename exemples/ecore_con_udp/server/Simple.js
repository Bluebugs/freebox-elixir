...
function _client_add(data, type, event)
{
   var tmp;

   elx.print("client add\n");

   tmp = ecore_con_client_server_get(event.client);
   if (tmp != server)
     {
	elx.print("not for me\n");
	return 1;
     }

   elx.print("client " + ecore_con_client_ip_get(event.client) + " just connected\n");

   ecore_con_client_send(event.client, "you are: " + ecore_con_client_ip_get(event.client) + "\n");

   return 0;
}

function _client_del(data, type, event)
{
   elx.print("client del\n");

   return 0;
}

function _client_data(data, type, event)
{
   var tmp;

   elx.print("client data\n");
   ...
   evas_textblock_cursor_text_append(cursor, ecore_con_client_ip_get(event.client) + ": " + event.data);
...
{
   handler = {
      add: ecore_event_handler_add(ECORE_CON_EVENT_CLIENT_ADD, _client_add, self),
      del: ecore_event_handler_add(ECORE_CON_EVENT_CLIENT_DEL, _client_del, self),
      data: ecore_event_handler_add(ECORE_CON_EVENT_CLIENT_DATA, _client_data, self)
   };

   server = ecore_con_server_add(ECORE_CON_REMOTE_UDP, "0::0", 42000, self);
   if (!server)
